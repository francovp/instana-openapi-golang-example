...
type ApiGetSliRequest struct {
	ctx        context.Context
	ApiService *SLIReportApiService
	sliId      string
	slo        *float64
	from       *int64
	to         *int64
...
func (r ApiGetSliRequest) Slo(slo float64) ApiGetSliRequest {
	r.slo = &slo
	return r
}

func (r ApiGetSliRequest) From(from int64) ApiGetSliRequest {
	r.from = &from
	return r
}

func (r ApiGetSliRequest) To(to int64) ApiGetSliRequest {
	r.to = &to
	return r
}

func (r ApiGetSliRequest) Execute() ([]SliReport, *http.Response, error) {
	return r.ApiService.GetSliExecute(r)
}
...
GetSli Generate SLI report
	...
	@param sliId
	@return ApiGetSliRequest
*/
func (a *SLIReportApiService) GetSli(ctx context.Context, sliId string) ApiGetSliRequest {
	return ApiGetSliRequest{
		ApiService: a,
		ctx:        ctx,
		sliId:      sliId,
...
//
//	@return []SliReport
func (a *SLIReportApiService) GetSliExecute(r ApiGetSliRequest) ([]SliReport, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SLIReportApiService.GetSli")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/sli/report/{sliId}"
	localVarPath = strings.Replace(localVarPath, "{"+"sliId"+"}", url.PathEscape(parameterToString(r.sliId, "")), -1)
	...
	if r.slo != nil {
		localVarQueryParams.Add("slo", parameterToString(*r.slo, ""))
	}
	if r.from != nil {
