...
func (r ApiGetEventRequest) Execute() (*EventResult, *http.Response, error) {
...
//
//	@return EventResult
func (a *EventsApiService) GetEventExecute(r ApiGetEventRequest) (*EventResult, *http.Response, error) {
	var (
...
func (r ApiGetEventsRequest) EventTypeFilters(eventTypeFilters []string) ApiGetEventsRequest {
	r.eventTypeFilters = &eventTypeFilters
	return r
}
...
func (r ApiGetEventsRequest) FilterEventUpdates(filterEventUpdates bool) ApiGetEventsRequest {
	r.filterEventUpdates = &filterEventUpdates
	return r
}

func (r ApiGetEventsRequest) Execute() ([]EventResult, *http.Response, error) {
...
//
//	@return []EventResult
func (a *EventsApiService) GetEventsExecute(r ApiGetEventsRequest) ([]EventResult, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.eventTypeFilters == nil {
		return localVarReturnValue, nil, reportError("eventTypeFilters is required and must be specified")
	}
	...
	if r.filterEventUpdates != nil {
		localVarQueryParams.Add("filterEventUpdates", parameterToString(*r.filterEventUpdates, ""))
	}
	{
		t := *r.eventTypeFilters
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				localVarQueryParams.Add("eventTypeFilters", parameterToString(s.Index(i), "multi"))
			}
		} else {
			localVarQueryParams.Add("eventTypeFilters", parameterToString(t, "multi"))
		}
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
...
type ApiGetEventsByIdsRequest struct {
	ctx         context.Context
	ApiService  *EventsApiService
	requestBody *[]string
}

func (r ApiGetEventsByIdsRequest) RequestBody(requestBody []string) ApiGetEventsByIdsRequest {
	r.requestBody = &requestBody
	return r
}

func (r ApiGetEventsByIdsRequest) Execute() ([]EventResult, *http.Response, error) {
	return r.ApiService.GetEventsByIdsExecute(r)
}

/*
GetEventsByIds Get events given a set of IDs

This endpoint retrieves events given a set of IDs. Only actually existing event IDs are respected in the result set.
...
- **eventIds:** The set of IDs of the events to fetch.
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiGetEventsByIdsRequest
*/
func (a *EventsApiService) GetEventsByIds(ctx context.Context) ApiGetEventsByIdsRequest {
	return ApiGetEventsByIdsRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
//
//	@return []EventResult
func (a *EventsApiService) GetEventsByIdsExecute(r ApiGetEventsByIdsRequest) ([]EventResult, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "EventsApiService.GetEventsByIds")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
